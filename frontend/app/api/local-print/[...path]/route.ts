  ...
  if (requestedPath === "print" && isCloudHost) {
    return new Response(
      JSON.stringify({
        status: "cloud",
        detail:
          "Direct agent printing is not available from a cloud-deployed frontend. Print jobs are automatically queued via the backend for the local PrimePOS Connector to claim.",
      }),
      {
        status: 503,
        headers: { "Content-Type": "application/json" },
