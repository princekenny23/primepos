...
export default function ItemSalesPage() {
  ...
  const loadReportData = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to load item sales:", error)
      toast({
        title: t("common.messages.error"),
        description: error?.message || "Failed to load item sales",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  const handleExport = async (formatType: "xlsx" | "pdf") => {
    ...
    try {
      await reportService.downloadReport(
        ...
        `item-sales-${startDate}-to-${endDate}.${formatType}`
      )
    } catch (error: any) {
      toast({
        title: t("common.messages.error"),
  ...
  return (
    <DashboardLayout>
      <PageLayout
        title="Item Sales"
        description="Top item gross sales and item-level performance"
      >
