...
import { AddEditBarTableModal } from "@/components/modals/add-edit-bar-table-modal"
...
export default function BarTablesPage() {
  ...
  const loadTables = useCallback(async () => {
    ...
    setIsLoading(true)
    try {
      const response = await tabService.listTables({ outlet: outlet.id })
  ...
  const handleDeleteTable = async () => {
    ...
    try {
      await tabService.deleteTable(selectedTable.id)
      toast({
        title: "Table Deleted",
  ...
  return (
    <DashboardLayout>
      <PageLayout
        ...
        description="Manage bar tables and seating"
      >
        {/* Action Buttons */}
